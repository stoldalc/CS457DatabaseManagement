...
-Buffer can consist of a stack or a list
-current Proccess int that is checked in


- begin transaction;
Make a buffer copy of the database that has changes

- commit;
Write the buffer copy to file
...
currentDB = ""
#The currentProcess variable is the index of the processDatabaseCopys currently being used
currentProcess = -1
processDatabaseCopys = []
                ...
                while databaseCopy[i] != "<<\n" :
                    ...
                    if databaseCopy[i][0] == '*':
                        ...
                        while databaseCopy[i][0] != '*':
                            ...
                            buffer.append(bufferValue)
                            i += 1
                            if i >= len(databaseCopy) or databaseCopy[i] == "<<\n" :
                                break
                        #appened the set of variables to the vars list
                        vars.append(buffer)
                        if i >= len(databaseCopy):
                            break
                break
    ...
    for i in range(len(matchPositions)):
        ...
        for j in range(len(databaseCopy)):
            ...
            #Check if the current line is equal to the RHS table
                ...
                while databaseCopy[k] != "<<\n":
                    ...
                    if databaseCopy[k][0] == "*":
                        ...
                        while databaseCopy[l][0] != "*" and databaseCopy[l] != "<<\n":
                            #if the current line is in the postion of a matched statment
                            if l == varPosition:
                                #Format the current line for printing
                                buffer = databaseCopy[l]
                                buffer = buffer.replace("$","")
                                buffer = buffer.replace("\n","")
                                buffer += "|"
                ...
                k = 0
                l = 0
        #The final formating for printing for the user
...
def selectWhereJoin(cmd):
    ...
    #This section 1356-1416 is all of the parsing for the command and getting 
    ...
    #Adding all of the variable instances to the custom object list
    for i in range(len(Tables)):
            Tables[i] = Tables[i].strip()
            tableName = Tables[i].split(" ")[0]
            tableVar = Tables[i].split(" ")[1]
    ...
    for i in range(len(LHSVarsInstances)):
        #For each right hand side
        for j in range(len(RHSVarsInstances)):
            #If the condition is satfied add the positions to a list of matched postions
            if LHSVarsInstances[i] == RHSVarsInstances[j]:
                matchFound = True
                buffer = []
                buffer.append(i)
                buffer.append(j)
                matchPositions.append(buffer)
        #If this is a left outer join add all of the variables from the left table
        # and then mark its second position as a -1 to mark it as no match found
    ...
    for i in range(len(matchPositions)):
        ...
        for j in range(len(databaseCopy)):
            ...
            #Check if the current line is equal to the RHS table
                    ...
                    if databaseCopy[k][0] == "*":
                        ...
                        while databaseCopy[l][0] != "*" and databaseCopy[l] != "<<\n":
                            if l == varPosition and matchPositions[i][1] != -1:
                                #Adding format to the specfic line
                                buffer = databaseCopy[l]
                                buffer = buffer.replace("$","")
                                buffer = buffer.replace("\n","")
                                buffer += "|"
                ...
                k = 0
                l = 0
        #Here is where the final informatiin is combined formated and printed for the user
        varInstancePrint += LHSPrint
        varInstancePrint += RHSPrint  
        varInstancePrint = varInstancePrint[:-1]    
        print(varInstancePrint)
...
def beginTransaction(cmd):
    ...
    #The case that the user is not within a database 
    if not inDataBase():
        print("-- !Failed to start transaction not currently in a database.")
        return
